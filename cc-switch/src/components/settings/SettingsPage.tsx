...
export function SettingsPage({
  ...
  const visibleTabs = useMemo(() => getVisibleSettingsTabs(), []);
  const [activeTab, setActiveTab] = useState<string>(() =>
    resolveVisibleSettingsTab("general"),
  );
  const [showRestartPrompt, setShowRestartPrompt] = useState(false);

  useEffect(() => {
    if (open) {
      setActiveTab(resolveVisibleSettingsTab(defaultTab));
      resetStatus();
    }
    ...
    <div className="flex flex-col h-full overflow-hidden px-6">
      {isBusy ? (
        ...
        >
          ...
          >
            {isSettingsTabVisible("general") && (
              <TabsTrigger value="general">
                {t("settings.tabGeneral")}
              </TabsTrigger>
            )}
            {isSettingsTabVisible("proxy") && (
              <TabsTrigger value="proxy">{t("settings.tabProxy")}</TabsTrigger>
            )}
            {isSettingsTabVisible("auth") && (
              <TabsTrigger value="auth">
                {t("settings.tabAuth", { defaultValue: "认证" })}
              </TabsTrigger>
            )}
            {isSettingsTabVisible("advanced") && (
              <TabsTrigger value="advanced">
                {t("settings.tabAdvanced")}
              </TabsTrigger>
            )}
            {isSettingsTabVisible("usage") && (
              ...
              <TabsContent value="general" className="space-y-6 mt-0">
                {settings ? (
                  <motion.div
                    ...
                    className="space-y-6"
                  >
                    {isGeneralSectionVisible("wellauAccount") && (
                      <WellauAccountSection />
                    )}
                    ...
                    {isGeneralSectionVisible("theme") && <ThemeSettings />}
                    {isGeneralSectionVisible("appVisibility") && (
                      <AppVisibilitySettings
                        settings={settings}
                        onChange={handleAutoSave}
                      />
                    )}
                    {isGeneralSectionVisible("skillStorage") && (
                    ...
                    {isGeneralSectionVisible("skillSync") && (
                    ...
                    {isGeneralSectionVisible("codexAuth") && (
                      <CodexAuthSettings
                        settings={settings}
                        onChange={handleAutoSave}
                      />
                    )}
                    {isGeneralSectionVisible("window") && (
                      <WindowSettings
                        settings={settings}
                        onChange={handleAutoSave}
                      />
                    )}
                    {isGeneralSectionVisible("terminal") && (
                      <TerminalSettings
              ...
              <TabsContent value="advanced" className="space-y-6 mt-0 pb-4">
                {settings ? (
                  ...
                  >
                    ...
                    >
                      {isAdvancedSectionVisible("directory") && (
                      <AccordionItem
                        value="directory"
                      ...
                      {isAdvancedSectionVisible("backup") && (
                      <AccordionItem
                        value="backup"
                        className="rounded-xl glass-card overflow-hidden"
                      >
                      ...
                      {isAdvancedSectionVisible("cloudSync") && (
                      <AccordionItem
                        value="cloudSync"
                        className="rounded-xl glass-card overflow-hidden"
                      >
                      ...
                      {isAdvancedSectionVisible("logConfig") && (
                      <AccordionItem
                        value="logConfig"
                        className="rounded-xl glass-card overflow-hidden"
                      >
