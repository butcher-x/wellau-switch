...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // Wellau: 禁用「启动时把本地 live 配置自动导入为 `default` 供应商」。
            // Wellau 的供应商只来自登录后的 Key 导入；自动导入会把 ~/.claude /
            // ~/.codex 里残留的 api.wellau.com 变成一个多余的 `default` 供应商。
            // 手动空状态导入按钮仍可用（command import_default_config 未移除）。
            ...
            // Wellau: 移除历史遗留的 Claude / Codex 官方供应商，以及自动导入产生的
            // `default` 供应商（产品上只展示登录导入的供应商）。
            remove_wellau_unwanted_providers(&app_state);

            // Wellau: 一次性回填既有数据库的路由 / 故障转移默认值（新装由 schema seed 处理）。
            match app_state.db.apply_wellau_proxy_defaults_once() {
                Ok(true) => log::info!("✓ Applied Wellau proxy defaults to existing DB"),
                Ok(false) => {}
                Err(e) => log::warn!("✗ Failed to apply Wellau proxy defaults: {e}"),
            }
        ...
        .invoke_handler(tauri::generate_handler![
            wellau_auth::wellau_login,
            wellau_auth::wellau_logout,
            wellau_auth::wellau_remove_imported_providers,
            wellau_auth::wellau_get_session,
            wellau_auth::wellau_refresh,
            wellau_auth::wellau_list_keys,
            ...
            // Wellau 环境安装（桌面应用 + Node 自举）
            commands::probe_desktop_apps,
            commands::install_desktop_app,
...
/// Wellau: 删除产品上不需要的内置 / 自动生成供应商：
/// - `claude-official` / `codex-official`：内置官方供应商（不再种子，清理老库残留）
/// - `default`（claude / codex）：启动自动导入 live 配置产生的多余供应商
///
/// 若目标正处于当前选中则先清空当前指针，保证幂等且不残留。
fn remove_wellau_unwanted_providers(state: &store::AppState) {
    use crate::app_config::AppType;

    let targets = [
        ("claude-official", AppType::Claude),
        ("codex-official", AppType::Codex),
        ("default", AppType::Claude),
        ("default", AppType::Codex),
    ];

    for (id, app_type) in targets {
        match state.db.get_provider_by_id(id, app_type.as_str()) {
            Ok(Some(_)) => {
                ...
                match state.db.delete_provider(app_type.as_str(), id) {
                    Ok(_) => log::info!("✓ Removed Wellau-unwanted official provider: {id}"),
                    Err(e) => log::warn!("✗ Failed to remove official provider {id}: {e}"),
                }
            }
            Ok(None) => {}
            Err(e) => log::warn!("✗ Failed to query official provider {id}: {e}"),
        }
    }
}

fn window_state_flags() -> StateFlags {
    // Wellau: 不保存/恢复窗口尺寸，保证每次启动都按配置的正方形比例呈现，
    // 避免 window-state 插件用历史尺寸覆盖方形窗口。
    StateFlags::POSITION
}
