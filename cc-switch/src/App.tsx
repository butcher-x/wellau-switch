...
import {
  getWellauExtension,
  wellauExtensions,
} from "@wellau/extensions/registry";
...
type WellauView = `wellau:${string}`;
type View = CoreView | WellauView;
...
const STORAGE_KEY = wellauBrand.lastAppStorageKey;
...
const VIEW_STORAGE_KEY = wellauBrand.lastViewStorageKey;
const VALID_VIEWS: CoreView[] = [
  "providers",
  "settings",
  "prompts",
  "skills",
...
const isWellauView = (view: View): view is WellauView =>
  view.startsWith("wellau:");

const getWellauView = (id: string): WellauView => `wellau:${id}`;

const getInitialView = (): View => {
  const saved = localStorage.getItem(VIEW_STORAGE_KEY) as View | null;
  if (saved && isWellauView(saved) && getWellauExtension(saved.slice(7))) {
    return saved;
  }
  if (saved && VALID_VIEWS.includes(saved as CoreView)) {
    return saved;
  }
  return "providers";
};
...
function App() {
  ...
  const visibleApps: VisibleApps =
    settingsData?.visibleApps ?? wellauBrand.defaultVisibleApps;
  ...
  const renderContent = () => {
    if (isWellauView(currentView)) {
      const extension = getWellauExtension(currentView.slice(7));
      const Panel = extension?.Panel;
      const content = Panel ? (
        <Panel activeApp={activeApp} />
      ) : (
        <div className="px-6 pt-4 text-sm text-muted-foreground">
          Wellau extension not found.
        </div>
      );
            ...
            {currentView !== "providers" ? (
              <div className="flex items-center gap-2">
                ...
                <h1 className="text-lg font-semibold">
                  ...
                  {isWellauView(currentView) &&
                    (getWellauExtension(currentView.slice(7))?.title ??
              ...
              <div className="flex items-center gap-2">
                ...
                />
                {wellauExtensions.map((extension) => (
                  <Button
                    key={extension.id}
                    variant="ghost"
                    size="sm"
                    onClick={() => setCurrentView(getWellauView(extension.id))}
                    title={extension.title}
                    className="hover:bg-black/5 dark:hover:bg-white/5"
                  >
                    {extension.toolbarLabel ?? extension.title}
                  </Button>
                ))}
